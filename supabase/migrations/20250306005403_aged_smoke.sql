/*
  ...
  1. New Tables
    - training_materials: Stores course content and metadata
    - training_progress: Tracks user progress through materials
    - training_feedback: Stores user ratings and comments
    - training_metrics: Records detailed usage metrics

  2. Security
    - RLS enabled on all tables
    - Policies for authenticated users
    - Security definer function for analytics

  3. Features
    - Automatic updated_at timestamps
    - Analytics function for progress tracking
...
CREATE INDEX IF NOT EXISTS idx_training_materials_role 
  ON training_materials(role);
CREATE INDEX IF NOT EXISTS idx_training_progress_user 
  ON training_progress(user_id);
CREATE INDEX IF NOT EXISTS idx_training_progress_material 
  ON training_progress(material_id);
CREATE INDEX IF NOT EXISTS idx_training_feedback_material 
  ON training_feedback(material_id);
CREATE INDEX IF NOT EXISTS idx_training_metrics_user 
  ON training_metrics(user_id);
...
-- Create trigger safely with IF NOT EXISTS
DO $$ BEGIN
